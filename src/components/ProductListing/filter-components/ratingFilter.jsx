...
const RatingFilter = () => {
    ...
    const minRatingHandler = (e) => productsDispatch({
        filterType: "MIN_RATING",
        minRating: e.target.value,
        progressStatusMinRating: e.target.value*100 / filtersData.ratingFilter.maxRating + "%"
    })
    
    const maxRatingHandler = (e) => productsDispatch({
        filterType: "MAX_RATING",
        maxRating: e.target.value,
        progressStatusMaxRating: (100 - e.target.value*100 / filtersData.ratingFilter.maxRating) + "%"
    })
      ...
      <ul className="outline-container rating-slider p-5 my-5 b-radius-2 flex-column flex-gap-1 flex-wrap w-100  my-5">
        <li className="no-list form-heading text-bold py-5 px-0">Rating</li>
          ...
          <article className="range-slider-container b-radius-1">
            <article className="range-slider-progress b-radius-1"
                style={{left: productsState.progressStatusMinRating, right: productsState.progressStatusMaxRating}} 
            ></article>
          </article>
