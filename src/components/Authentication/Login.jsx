...
const Login = () => {
    ...
    const setEmailHandler = (e) => loginDispatch({email: e.target.value});
    const setPasswordHandler = (e) => loginDispatch({password: e.target.value});
    ...
    return (
        <form onSubmit={(e) => loginHandler(e,location,navigate)} className="input-form login flex-column flex-gap-1 flex-wrap h-auto w-100">
            <section className={`input-container flex-column m-5 ${loginState.email.length > 0 || loginState.focus.email ? "focused":"" }`}>
                <input id="email" className="textbox-content p-5" type="email" name="email" onChange={setEmailHandler} value={loginState.email} onFocus={() => loginDispatch({focus : {email: true, password:false}})} onBlur={()=> loginDispatch({focus : {email: false, password:false}})}/>
            ...
            <section className={`input-container flex-column m-5 ${loginState.password.length > 0 || loginState.focus.password ? "focused":"" }`}>
                <input id="password" className="textbox-content p-5" type={`${ loginState.showPassword ? "text" : "password" }`} name="password" onChange={setPasswordHandler} value={loginState.password} onFocus={() =>  loginDispatch({focus : {email: false, password:true}})} onBlur={()=> loginDispatch({focus : {email: false, password:false}})}/>
